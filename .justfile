...
sbom:
	docker run --rm --user=$(id -u):$(id -g) --volume=$PWD:$PWD --workdir=$PWD ghcr.io/cyclonedx/cdxgen-debian-rust:v11.4.3 --fail-on-error --author="$(git config --get user.name) <$(git config --get user.email)>"
